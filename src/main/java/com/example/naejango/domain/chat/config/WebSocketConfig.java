...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
     ...
     * Redis 는 STOMP 프로토콜을 지원하지 않기 때문에 해당 Simple MessageBroker 를 사용하는데,
     * 해당 브로커로 연결 이후 로직을 커스터마이징 하여 메세지 브로커로서 동작하게 해야한다.
     * STOMP 프로토콜을 지원하는 RabbitMQ, ActiveMQ 같은 경우 enableStompBrokerRelay 라는 설정으로
     * 외부의 메세지 브로커를 직접 갖다 쓸 수 있는데 간단한 설정으로 다양한 기술을 사용할 수 있다.
